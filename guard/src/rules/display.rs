...
impl Display for ValueOnlyDisplay {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match &*self.0 {
            ...
            PathAwareValue::Int((_path, value)) => {
                formatter.write_fmt(format_args!("{}", value))?
            }
            PathAwareValue::Float((_path, value)) => {
                formatter.write_fmt(format_args!("{}", value))?
            }
            PathAwareValue::Char((_path, value)) => {
                formatter.write_fmt(format_args!("\'{}\'", value))?
            }
            PathAwareValue::List((_path, list)) => {
                ...
                if !list.is_empty() {
                    ...
                    for (idx, each) in list.iter().enumerate() {
                        ValueOnlyDisplay(Rc::new(each.clone())).fmt(formatter)?;
                        if last != idx {
                            formatter.write_str(",")?;
                        }
                    }
            ...
            PathAwareValue::Map((_path, map)) => {
                ...
                if !map.is_empty() {
                    ...
                    for (idx, (key, value)) in map.values.iter().enumerate() {
                        ...
                        formatter.write_str(":")?;
                        ValueOnlyDisplay(Rc::new(value.clone())).fmt(formatter)?;
                        if last != idx {
                            formatter.write_str(",")?;
                        }
                    }
...
impl Display for PathAwareValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        ValueOnlyDisplay(Rc::new(self.clone())).fmt(f)
    }
}
        ...
        match self {
            ...
            ClauseCheck::NoValueForEmptyCheck(message) => {
                f.write_fmt(format_args!(
                    ...
                    Status::FAIL,
                    message.as_ref().map_or("", |s| s.as_ref())
                ))?;
            }
            ...
            ClauseCheck::MissingBlockValue(missing) => {
                f.write_fmt(format_args!(
                    ...
                    missing
                        ...
                        .map_or(String::from(""), |p| (*p).self_path().0.to_string())
                ))?;
            }
            ...
            ClauseCheck::DependentRule(dependent) => {
                f.write_fmt(format_args!(
                    ...
                    dependent.rule, dependent.status,
                ))?;
            }
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RecordType::TypeCheck(block_check) => {
                f.write_fmt(format_args!(
                    "Type({}, Status={})",
                    block_check.type_name, block_check.block.status
                ))?;
            }
